...
series2 = read_excel('EAFVdata_32049072.xlsx', sheet_name='CalAdj', header=0, index_col=0, parse_dates=True, squeeze=True)
...
# ===================================
# Model 4: Holt-Winter method with additive trend and seasonality and Damped values and BoxCox Fit
...
# Here, the parameters alpha = 0.4, beta = 0.0, and gamma = 0.4
# ===================================
fit3 = ExponentialSmoothing(series, seasonal_periods=12, trend='add', seasonal='mul').fit(smoothing_level = 0.4, smoothing_slope=0.0,  smoothing_seasonal=0.4)
new_fit3, new_forecast3 = backTransform_CalAdj(fit3.forecast(12), fit3, 1)


print("\n********************************\nForecasting Retail Sales Index (EAFV) with Holt-Winters method\n")
#=====================================
# Time and forecast plots
#=====================================

org_series.plot(label = 'Time plot of original series', legend=True)
new_forecast1.plot(label = 'Model 1: Opt HW-Multiplicative seasonality',color='red', legend=True)
new_fit1.fittedvalues.plot(color='red',legend=False)

pyplot.xlabel('Year')
pyplot.ylabel('Values')
pyplot.title('HW method-based forecasts for EAFV Data')
pyplot.show()

org_series.plot(label = 'Time plot of original series', legend=True)
new_forecast2.plot(label = 'Model 2: Opt HW-Multiplicative seasonality_Damped',color='green', legend=True)
...
org_series.plot(label = 'Time plot of original series', legend=True)
new_forecast2.plot(label = 'Model 3:\nHW-Multiplicative seasonality_\nAlpha=0.4,\nBeta=0.0,Gamma=0.4',color='green', legend=True)
